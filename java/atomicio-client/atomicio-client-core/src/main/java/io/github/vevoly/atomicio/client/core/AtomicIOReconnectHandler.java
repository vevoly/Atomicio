...
public void channelInactive(ChannelHandlerContext ctx) throws Exception {
    ...
    if (config.isReconnectEnabled() && reconnectAttempts == 0) {
        log.warn("Connection lost. Starting reconnect process...");
        reconnectAttempts++;
    ...
    private void scheduleReconnect(AtomicIOClientConfig config) {
        ...
        client.getEventLoopGroup().schedule(() -> {
            ...
            client.fireReconnectingEvent(reconnectAttempts, (int) finalDelay);

            client.getBootstrap().connect(config.getHost(), config.getPort())
